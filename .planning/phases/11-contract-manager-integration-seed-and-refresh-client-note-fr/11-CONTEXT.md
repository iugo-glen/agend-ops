...
### Post-Research Refinements (2026-05-02, after RESEARCH.md)

These supersede earlier D-XX entries where they conflict. Decided with Glen after the researcher discovered that several CM MCP tools assumed by CONTEXT.md do not exist on the CM side today.

- **D-C2-REVISED: Sites section DEFERRED.** CM MCP does not expose `sites[]` (Site table exists in Prisma but no MCP tool surfaces it). Phase 11 ships with frontmatter `sites: []` always, and **no Sites managed section is rendered** — neither the marker pair nor the placeholder body. When CM later exposes `list_sites_for_client(clientId)`, a Phase 11.x phase will add the section and populate the field. Section order from D-C1 collapses by one (no Sites between Open Items and Usage).
- **D-C3-REVISED: Usage section uses `get_sla_status` (per-client), NOT `get_utilization_summary` / `get_capacity_summary` (which are team-wide globals).** `get_sla_status` returns a `projects[]` array with `clientName` for grouping and per-project hours-vs-budget. The Usage section renders the per-client subset of projects with: hours billed this month, hours YTD (if exposed), % of budget consumed. CONTEXT.md tool list in `<canonical_refs>` is corrected accordingly (replace `operations.ts` references to utilization/capacity with the `get_sla_status` reference for per-client Usage).
- **D-B-MOD-REVISED: `deployed_modules` derived from `activeContracts[].name`.** CM MCP does not expose per-Product detail today. Render `deployed_modules` as `sorted({contract.name for contract in activeContracts})`. Add a one-line note inside the rendered managed section (when one exists) or in vault_writer documentation that says `(derived from contract names)` so the fidelity limit is documented in-band. When CM later exposes per-Product detail, Phase 11.x can refine.
- **D-A3-REVISED: `warning` level added to `schemas/feed-entry.json` as Wave 0 task.** D-A3's `system`/`warning` shape is preserved verbatim. The schema enum gains `"warning"` alongside `["critical", "info", "debug"]`. This is an additive enum change (no breaking impact on existing entries). Wave 0 sequencing: schema edit MUST land before any code path emits a `warning`-level entry.
- **D-G1: `cm_client_id` mapping pass is MANDATORY (Wave 0), not deferred.** `get_client_summary` requires `clientId: number`, not domain. The mapping pass — `vault_writer --mode map-cm-clients` — calls `search_clients` per domain in `clients.jsonl` and writes back `cm_client_id` to each record. Idempotent (re-runs are safe; updates IDs if CM IDs ever change). New `clients.jsonl` records without `cm_client_id` cause the next sync to attempt a single `search_clients` for that domain only, then continue.

### Claude's Discretion
- HTTP client library choice (urllib stdlib vs `requests` add to requirements.txt — pick the lower-dependency option that supports retries cleanly; researcher recommends stdlib urllib — Phase 10 zero-extra-dep precedent)
